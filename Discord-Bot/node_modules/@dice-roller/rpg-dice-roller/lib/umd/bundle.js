...
(function (global, factory) {
  ...
  class CompareOperatorError extends TypeError {
    ...
    constructor(operator) {
      super(`Operator "${operator}" is invalid`); // Maintains proper stack trace for where our error was thrown (only available on V8)
  ...
  class DataFormatError extends Error {
     ...
     */
    constructor(data) {
      super(`Invalid data format: ${data}`); // Maintains proper stack trace for where our error was thrown (only available on V8)
  ...
  class DieActionValueError extends Error {
    ...
    constructor(die) {
      ...
      super(`Die "${die}" must have more than 1 possible value to ${action || 'do this action'}`); // Maintains proper stack trace for where our error was thrown (only available on V8)
  ...
  class NotationError extends Error {
    ...
    constructor(notation) {
      super(`Notation "${notation}" is invalid`); // Maintains proper stack trace for where our error was thrown (only available on V8)
  ...
  class RequiredArgumentError extends Error {
    ...
    constructor() {
      ...
      super(`Missing argument${argumentName ? ` "${argumentName}"` : ''}`); // Maintains proper stack trace for where our error was thrown (only available on V8)
  ...
  class ExplodeModifier extends ComparisonModifier {
    ...
    constructor() {
      ...
      this[penetrateSymbol] = !!penetrate; // set the modifier's sort order
    ...
    run(results, _context) {
      ...
      parsedResults.rolls = results.rolls.map(roll => {
        const subRolls = [roll];
        let compareValue = roll.value; // explode if the value matches the compare point, and we haven't reached the max iterations

        for (let i = 0; i < this.maxIterations && this.isComparePoint(compareValue); i++) {
          const prevRoll = subRolls[subRolls.length - 1]; // roll the dice

          const rollResult = _context.rollOnce(); // update the value to check against
  ...
  _defineProperty(Description, "types", {
    MULTILINE: 'multiline',
    INLINE: 'inline'
  });
  ...
  class ReRollModifier extends ComparisonModifier {
    ...
    constructor() {
      ...
      super(comparePoint);
      this.once = !!once; // set the modifier's sort order
    ...
    run(results, _context) {
      ...
      results.rolls.map(roll => {
        ...
        for (let i = 0; i < this.maxIterations && this.isComparePoint(roll.value); i++) {
          // re-roll the dice
          const rollResult = _context.rollOnce(); // update the roll value (Unlike exploding, the original value is not kept)
          // eslint-disable-next-line no-param-reassign


          roll.value = rollResult.value; // add the re-roll modifier flag

          roll.modifiers.add(`re-roll${this.once ? '-once' : ''}`); // stop the loop if we're only re-rolling once
  ...
  class StandardDice extends HasDescription {
    ...
    roll() {
      ...
      const rollResult = new RollResults(); // loop for the quantity and roll the die
  ...
  class CriticalFailureModifier extends ComparisonModifier {
     ...
     */
    constructor(comparePoint) {
      super(comparePoint); // set the modifier's sort order
  ...
  class KeepModifier extends Modifier {
    ...
    constructor() {
      ...
      super();
      this.end = end;
      this.qty = qty; // set the modifier's sort order
    ...
    run(results, _context) {
      ...
      if (results instanceof ResultGroup) {
        ...
        if (modifiedRolls.length === 1 && modifiedRolls[0] instanceof ResultGroup) {
          ...
          rollIndexes = [...modifiedRolls] // get a list of objects with roll values and original index
          .map((roll, index) => ({
            value: roll.value,
            index
          }));
      ...
      rollIndexes = rollIndexes // sort the list ascending by value
      .sort((a, b) => a.value - b.value).map(rollIndex => rollIndex.index) // get the roll indexes to drop
      .slice(...this.rangeToDrop(rollIndexes)); // loop through all of our dice to drop and flag them as such
  ...
  class DropModifier extends KeepModifier {
    ...
    constructor() {
      ...
      let qty = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
      super(end, qty); // set the modifier's sort order
  ...
  class MaxModifier extends Modifier {
    ...
    constructor(max) {
      super();
      this.max = max; // set the modifier's sort order
  ...
  class MinModifier extends Modifier {
    ...
    constructor(min) {
      super();
      this.min = min; // set the modifier's sort order
  ...
  class SortingModifier extends Modifier {
    ...
    constructor() {
      ...
      super();
      this.direction = direction; // set the modifier's sort order
  ...
  class TargetModifier extends ComparisonModifier {
    ...
    constructor(successCP) {
      ...
      super(successCP); // set the failure compare point

      this.failureComparePoint = failureCP; // set the modifier's sort order
    ...
    toJSON() {
      const {
        failureComparePoint,
        successComparePoint
      } = this; // get the inherited object, but remove the comparePoint property
  ...
  function peg$parse(input, options) {
    ...
    var peg$e64 = peg$otherExpectation("whitespace or comment");
  ...
  class DiceRoll {
    ...
    get maxTotal() {
      ...
      if (!this[maxTotalSymbol]) {
        ...
        const rolls = this[rollMethodSymbol](engines.max); // calculate the total
    ...
    get minTotal() {
      ...
      if (!this[minTotalSymbol]) {
        ...
        const rolls = this[rollMethodSymbol](engines.min); // calculate the total
  ...
  class DiceRoller {
    ...
    roll() {
      ...
      const rolls = filteredNotations.map(notation => {
        const diceRoll = new DiceRoll(notation); // add the roll log to our global log

        this[logSymbol].push(diceRoll); // return the current DiceRoll
